...
open class Narration<Index : Any>(
    ...
    internal val backStack by lazy {
        val backStack = BackStack(mutableStateListOf(currentPage))
    ...
    internal fun begin() {
        ...
        indexes = remember { backStack.indexes }
...
fun <Index : Any, D> Narration<Index>.turnTo(newPage: Index, data: D) {
    lifeCycleScope.launch {
        ...
        currentChapter.contentFactory?.onCreateData?.set(newPage as Any, mutableStateOf(data))
    }
}
 ...
 * to protocol buffers
 * */
fun <Index : Any, D> Narration<Index>.previousChapter(data: D? = null) {
    lifeCycleScope.launch {
        if (backStack.indexes.size > 1) {
            backStack.previous(onNarrationEnd)
...
fun <Index : Any> Narration<Index>.previousChapter() = previousChapter<Index, Any>(null)
